...
double foo(double x) { return (x > 0) ? 1 : 0; }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << (my_matrix.array() > 0).all()
            << std::endl; // if all elemnts are positive
  std::cout << (my_matrix.array() > 2).any()
            << std::endl; // if any element is greater than 2
  std::cout << (my_matrix.array() > 1).count()
            << std::endl; // count the number of elements greater than 1
  ...
  std::cout << "roll is Pi/" << M_PI / atan2(rmat(2, 1), rmat(2, 2))
            << std::endl;
  std::cout << "pitch: Pi/"
            << M_PI / atan2(-rmat(2, 0), std::pow(rmat(2, 1) * rmat(2, 1) +
                                                      rmat(2, 2) * rmat(2, 2),
                                                  0.5))
            << std::endl;
  std::cout << "yaw is Pi/" << M_PI / atan2(rmat(1, 0), rmat(0, 0))
            << std::endl;
  ...
  */
  ...
  std::cout << "MAT1:" << std::endl
            << mat1.unaryExpr(std::ptr_fun(foo)) << std::endl;
