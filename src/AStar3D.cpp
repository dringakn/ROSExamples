...
AStar3D::AStar3D(bool occupied_space, bool free_space, bool unknown_space, ufo::map::DepthType min_depth)
{
...
bool AStar3D::findNearestVoxel(ufo::map::Point3 position, ufo::map::Point3 *result)
{
    // Find the closest occupied voxel to position at depth 0
    for (auto it = map->beginNNLeaves(position,
    ...
    return false; // No neighbour found.
}
...
{
    ufo::map::Point3 pt;
    if (findNearestVoxel(ufo::map::Point3(s.point.x, s.point.y, s.point.z), &pt))
    {
        start.point.x = pt.x();
        start.point.y = pt.y();
        start.point.z = pt.z();
...
{
    ufo::map::Point3 pt;
    if (findNearestVoxel(ufo::map::Point3(g.point.x, g.point.y, g.point.z), &pt))
    {
        start.point.x = pt.x();
        start.point.y = pt.y();
        start.point.z = pt.z();
 ...
 *
 * @param bounding_volume
 */
void AStar3D::setBounds(ufo::geometry::BoundingVar *bounding_volume)
{
    // return path;
}
...
int AStar3D::shortestPath()
{
    ...
    ufo::geometry::Point size = map->getKnownBBX().getMax() - map->getKnownBBX().getMin();
    int width = size.x();
    int height = size.y();
    int depth = size.z();
