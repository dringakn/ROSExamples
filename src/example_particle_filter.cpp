...
geometry_msgs::PoseStamped poseEstimate;  // Estimated (weighted average) particle
...
void meanParticle()
{
  ...
  poseEstimate.pose.position.x = poseEstimate.pose.position.y = poseEstimate.pose.position.z = 0;
...
void publishParticles()
{
  ...
  std::stringstream ss;
  ss << std::setprecision(2) << poseEstimate.pose.position.x << " ," << poseEstimate.pose.position.y << " ,"
  ...
  distLM.points[0] = distLM.points[2] = distLM.points[4] = distLM.points[6] = distLM.points[8] = distLM.points[10] =
      poseEstimate.pose.position;
...
void particleFilter_Initialization()
{
  ...
  for (unsigned int i = 0; i < NP; i++)
  {
    ...
    Particles.poses[i].orientation = tf::createQuaternionMsgFromYaw(rng.uniformReal(0, 2 * M_PI));
    W[i] = 1.0 / NP;
  }
  publishParticles();
}
...
double measurement_Probability(geometry_msgs::Pose X, std::vector<double> Z)
{
  ...
  {
    expectedRange = sqrt(pow(X.position.x - Map[i].x, 2.0) + pow(X.position.y - Map[i].y, 2.0));
...
void particleFilter_ReSample()
{
  ...
  unsigned int idx = rng.uniformInteger(0, NP - 1);  // Generate random sample index
  ...
  for (unsigned int k = 0.98 * NP; k < NP; k++)
  {
    ...
    Particles.poses[k].orientation = tf::createQuaternionMsgFromYaw(rng.uniformReal(0, 2 * M_PI));
    W[k] = 1.0 / NP;
  }
  double sumW = 0;
  for (unsigned int k = 0; k < NP; k++)
...
{
  // Initialize the node
  ros::init(argc, argv, "ros_particle_filter_node_for_localizing_robot_in_a_vrep_maze");
  ros::NodeHandle nh;
  // Subscribe to odometry and range measurements
  ros::Subscriber subMeas = nh.subscribe("/myRobot/measurements", 10, callback_Measurements);
  ros::Subscriber subCmds = nh.subscribe("/myRobot/odometry", 10, callback_Odometry);
  // Publish topics
  pubLandMarks = nh.advertise<visualization_msgs::Marker>("LandMarks", 1, true);
  pubMeasurements = nh.advertise<visualization_msgs::Marker>("Measurements", 1, true);
  pubParticles = nh.advertise<geometry_msgs::PoseArray>("Particles", 1, true);
  pubEstimate = nh.advertise<geometry_msgs::PoseStamped>("EstimatedPose", 1, true);
  pubEstimateTxt = nh.advertise<visualization_msgs::Marker>("EstimatedPoseTxt", 1, true);
